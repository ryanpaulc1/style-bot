...
export async function addCommand(styleName, options) {
    ...
    if (isAuthRequired()) {
        ...
        if (!authResult.styles.includes(styleName)) {
            ...
            if (styleInfo.price) {
                ui.info(`Purchase at https://tokenatelier.dev/styles/${styleName} (${ui.formatPrice(styleInfo.price.single, styleInfo.price.currency)})`);
            }
            process.exit(1);
        }
    ...
    let tailwindVersion = info.tailwindVersion;
    if (options.tailwind) {
    ...
    let componentResolution = 'overwrite';
    let cssResolution = 'replace';
    if (conflicts.hasComponentConflicts && !options.force) {
        componentResolution = await promptComponentConflict(conflicts.conflictingComponents, options.force);
    ...
    ui.listItem(cssResolution === 'separate'
