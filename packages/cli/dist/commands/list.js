...
import { fetchAllStyleInfo } from '../lib/fetch.js';
...
export async function listCommand() {
    ...
    for (const style of styles) {
        ...
        const priceText = style.price
            ...
            : 'Free';
        console.log();
        console.log(`${accessIcon} ${ui.chalk.bold(style.title)} ${ui.chalk.gray(`(${style.name})`)}`);
        console.log(`  ${ui.chalk.gray(style.description)}`);
        console.log(`  ${ui.formatAccess(style.access)} • ${priceText}`);
    }
    console.log();
    // Show auth status
    if (isAuthRequired()) {
        const apiKey = await getApiKey();
        if (apiKey) {
            ui.info(`You have access to ${accessibleStyles.length} of ${styles.length} styles`);
        }
        else {
