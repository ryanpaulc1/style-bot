 ...
 */
export async function checkConflicts(info) {
    const result = {
 ...
 */
export async function promptComponentConflict(conflictingFiles, force) {
    if (force) {
        return 'overwrite';
    }
 ...
 */
export async function promptCssConflict(force) {
    if (force) {
        return 'replace';
    }
    const { resolution } = await inquirer.prompt([
